...
export const OUTPUT = "dist/component-registry.json";
...
async function main() {
  ...
  const [exports, themeTokens, docIndex, packageJson, maturity] = await Promise.all([
  ...
  for (const entry of exports) {
    ...
    components.push({
      name: entry.name,
      kind: entry.kind,
      category: categoryFor(entry.sourceFile),
      maturityTier: assignment?.tier ?? null,
      maturitySource: assignment?.source ?? null,
      maturityNote: assignment?.note ?? null,
      maturityNeedsReview: assignment ? assignment.source.startsWith("inferred:") : true,
      props: entry.props,
  ...
  const output = {
    $comment:
      "Generated by scripts/generate-component-registry.mjs during npm run build. Do not edit by hand. maturityTier is reconciled from registry/component-maturity.json, which is human-owned.",
    generatedAt: new Date().toISOString(),
    packageName: packageJson.name,
    packageVersion: packageJson.version,
    tierVocabulary: ["stable", "beta", "experimental", "internal", "deprecated"],
    components,
  };

  await fs.writeFile(path.join(root, OUTPUT), `${JSON.stringify(output, null, 2)}\n`);

  const counts = components.reduce((totals, component) => {
    totals[component.maturityTier ?? "unassigned"] = (totals[component.maturityTier ?? "unassigned"] ?? 0) + 1;
    return totals;
  }, {});

  console.log(`Generated ${OUTPUT} with ${components.length} records.`);
  console.log(
